...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Este endpoint ahora solo acepta solicitudes POST

    // Verifica si se proporcionó un nombre en la solicitud POST
    if (isset($_POST['id'])) {
        // Obtén el nombre de usuario de la solicitud POST
        ...
        if ($result->num_rows == 1) {
            ...
            $sql = "SELECT * FROM healthy_conditions WHERE users_id = ?";
            $stmt = $link->prepare($sql);
            ...
            if (empty($data)) {
                ...
                $response = array('code' => '1', 'message' => 'No se encontraron consignaciones para el usuario.');
            } else {
        ...
        // Nombre no proporcionado en la solicitud POST
