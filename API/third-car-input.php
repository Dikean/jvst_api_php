...
{//Este endpoint es solo de tipo post
    ...
    if (isset($_POST['where_to_delivery']) && trim($_POST['where_to_delivery']) != ''  && isset($_POST['delivery_address']) && trim($_POST['delivery_address']) &&  isset($_POST['delivery_city_code']) && trim($_POST['delivery_city_code'])   && isset($_POST['id']) && trim($_POST['id']))
    { //se validan que los parámetros no vayan vacíos



        $sql = "UPDATE car SET where_to_delivery = ?, delivery_address = ?, delivery_city_code = ?, initial_delivery_date = ?, end_delivery_date = ?, initial_delivery_initial_time = ?, initial_delivery_end_time = ?, end_delivery_initial_time = ?, end_delivery_end_time = ? WHERE id = ?"; //se escribe la consulta para la sentencia preparada
        $stmt = $link->prepare($sql); //se corre la sentencia

        $stmt->bind_param("ssssssssss", $_POST['where_to_delivery'], $_POST['delivery_address'], $_POST['delivery_city_code'], $_POST['initial_delivery_date'], $_POST['end_delivery_date'], $_POST['initial_delivery_initial_time'], $_POST['initial_delivery_end_time'], $_POST['end_delivery_initial_time'], $_POST['end_delivery_end_time'], $_POST['id']);
        $result=$stmt->execute();
