...
{//Este endpoint es solo de tipo post
    ...
    //Obtener el id
    $sql = "SELECT id, municipality_description,capital_description FROM municipalities ORDER BY municipality_description ASC "; // SQL with parameters
  ...
  //  $data = $result->fetch_assoc(); // fetch data  
    ...
    else
    {
        ...
        {
            $row =  array('id' => $data['id'] ,'municipio' => $data['municipality_description'], 'capital' => $data['capital_description']);
            array_push($obj, $row);
        }
...
else 
{
    ...
    $header = "HTTP/1.1 405 Method Not Allowed"; // en caso de que los campos vayan vacíos, esta es la respuesta que arroja el endpoint.
    $response = array('code' => '1','message' => 'Method Not Allowed, Método no soportado para este endpoint.');
