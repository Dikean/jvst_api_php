...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {//Este endpoint es solo de tipo post
    ...
    //Obtener el id
    $sql = " SELECT name, last_name,nit,date,email,owner,observacion,nit_usuario,owner_bill.email_usuario, purchase_order.`id`,purchase_date,installments AS cuotas,last_advice_date AS fecha_renovacion,
amount,payment_date,owner_bill.payment_reference,transaction_id,payments.id AS payment_id,payments.`order_id`
 FROM owner_bill
 JOIN purchase_order ON (owner_bill.`payment_reference` = purchase_order.`payment_reference`)
JOIN payments ON(purchase_order.`payment_reference` = payments.`payment_reference`)
 WHERE owner_bill.`email_usuario` = ?  AND payments.`id` = ?  ORDER BY purchase_date DESC"; // SQL with parameters
    $stmt = $link->prepare($sql);
    $stmt->bind_param("ss",  $_GET['email_usuario'], $_GET['payment_id']);
    $stmt->execute();
